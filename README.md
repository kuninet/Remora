...
1. [Releases](https://github.com/kuninet/Remora/releases) から最新の `Remora-vX.Y.zip` をダウンロード
2. 展開して `Remora.app` を `/Applications` にドラッグ
3. **初回だけ、ターミナルで以下を 1 回実行** (詳細は下の「⚠ 初回起動でエラーが出る件」参照)

   ```bash
   xattr -dr com.apple.quarantine /Applications/Remora.app
   ```

4. `/Applications/Remora.app` をダブルクリックで起動

### ⚠ 初回起動でエラーが出る件

Remora は未署名アプリ (Apple Developer Program に未加入) です。何もせずダブルクリックすると macOS が次のエラーを出すことがあります:

> "Remora" は壊れているため開けません。ゴミ箱に入れる必要があります。

または:

> Apple は、"Remora.app" にマルウェアが含まれていないことを検証できませんでした。

**ファイルは壊れていません**。Web ブラウザ経由でダウンロードしたファイルに macOS が付ける `com.apple.quarantine` 拡張属性が原因です。上記の `xattr -dr ...` コマンドで属性を外せば普通に起動できます。**1 回外せば二度と出ません** (再ダウンロード/再インストール時にまた実行する必要あり)。

その他のダイアログ (SMB 接続、キーチェーン許可) については下の「初回起動時の注意」を参照。
...
初回マウントを実行すると macOS から次のダイアログが順に出ます。
